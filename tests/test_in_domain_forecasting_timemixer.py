...
from forts.model_pipeline.model_pipeline import AutoUnivariateTimeMixer, ModelPipeline
...
def setup_pipeline():
    ...
    mp.MODEL_LIST = [("AutoTimeMixer", AutoUnivariateTimeMixer)]
    return dp, mp
...
def test_in_domain_forecasting(setup_pipeline):
    ...
    assert (
        row_forecast["Forecast SMAPE MEAN (last window) Per Series_in_domain"]
        is not None
    )
    assert not np.isnan(
        row_forecast["Forecast SMAPE MEAN (last window) Per Series_in_domain"]
