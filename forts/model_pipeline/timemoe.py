...
from forts.model_pipeline.configuration_time_moe import TimeMoeConfig
from forts.model_pipeline.models.ts_generation_mixin import TSGenerationMixin
...
class TimeMoeSparseExpertsLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        """
        Forward pass for the Sparse Experts Layer.
        """
...
class TimeMoeForPrediction(TimeMoePreTrainedModel, TSGenerationMixin):
    """
    TimeMOE model with a prediction head for forecasting.

    This class wraps the TimeMoeModel and adds a linear layer (or multiple heads)
    to generate forecasts for specified horizon lengths. It also handles the
    calculation of the training loss, including the auxiliary load balancing loss
    for the Mixture-of-Experts layers.
    """
...
class TimeMOE(BaseModel):
    ...
    hidden_size : int, optional
        Dimension of the hidden layers, by default 128.
    intermediate_size : int, optional
        Dimension of the intermediate layers in the feed-forward networks, by default 512.
    num_hidden_layers : int, optional
        Number of hidden layers in the model, by default 2.
    num_attention_heads : int, optional
        Number of attention heads, by default 4.
    num_key_value_heads : int, optional
        Number of key-value heads for grouped query attention, by default 1.
    hidden_act : str, optional
        Activation function for the hidden layers, by default "silu".
    max_position_embeddings : int, optional
        Maximum sequence length for position embeddings, by default 512.
    initializer_range : float, optional
        Standard deviation for weight initialization, by default 0.02.
    rms_norm_eps : float, optional
        Epsilon value for RMS normalization, by default 1e-6.
    use_cache : bool, optional
        Whether to use caching for faster inference, by default True.
    rope_theta : float, optional
        Theta value for Rotary Position Embeddings, by default 10000.0.
    attention_dropout : float, optional
        Dropout rate for the attention layers, by default 0.1.
    num_experts : int, optional
        Number of experts in the MoE layers, by default 4.
    num_experts_per_tok : int, optional
        Number of experts to use for each token, by default 2.
    use_dense : bool, optional
        Whether to use a dense feed-forward network instead of MoE layers, by default False.
    apply_aux_loss : bool, optional
        Whether to apply the auxiliary load balancing loss for MoE, by default True.
    router_aux_loss_factor : float, optional
        Factor for the auxiliary loss, by default 0.01.
    horizon_lengths : list of int, optional
        List of horizon lengths for multi-horizon forecasting, by default None.
    loss : PyTorch module, optional
        Loss function to use for training, by default MAE().
    valid_loss : PyTorch module, optional
        Loss function to use for validation, by default None.
    max_steps : int, optional
        Maximum number of training steps, by default 1000.
    learning_rate : float, optional
        Learning rate for the optimizer, by default 0.001.
    num_lr_decays : int, optional
        Number of learning rate decays, by default -1.
    early_stop_patience_steps : int, optional
        Number of steps to wait for improvement before early stopping, by default -1.
    val_check_steps : int, optional
        Number of training steps between validation checks, by default 100.
    batch_size : int, optional
        Number of time series in each batch, by default 32.
    valid_batch_size : int, optional
        Number of time series in each validation batch, by default None.
    windows_batch_size : int, optional
        Number of windows to sample in each batch, by default 1024.
    inference_windows_batch_size : int, optional
        Number of windows for inference, by default 1024.
    start_padding_enabled : bool, optional
        Whether to pad the start of the time series, by default False.
    scaler_type : str, optional
        Type of scaler to use for normalization, by default "robust".
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = TimeMoeConfig(
            ...
            horizon_lengths=horizon_lengths if horizon_lengths is not None else [h],
